...
import com.utils.CmdLineOptions;
...
public class TestApp {
    ...
    public static void main(String[] args) {
        ...
        restAbbyyClient.serverUrl = "http://cloud-eu.ocrsdk.com";
        restAbbyyClient.applicationId = ClientConfiguration.APPLICATION_ID;
        restAbbyyClient.password = ClientConfiguration.PASSWORD;
    ...
    private static boolean checkAppId() {
        String appId = ClientConfiguration.APPLICATION_ID;
        String password = ClientConfiguration.PASSWORD;
        if (appId.isEmpty() || password.isEmpty()) {
            System.out
                    .println("Error: No application id and password are specified.");
            System.out.println("Please specify them in com.net.ClientSettings.java.");
            return false;
        }
        return true;
    }
    ...
    private static void displayHelp() {
        System.out
                .println("This program is able to recognize:\n"
                        ...
                        + "  java com.net.TestApp recognize testImage.jpg result.xml\n"
                        + "  java com.net.TestApp recognize page1.jpg page2.jpg page3.jpg result.pdf --lang=French,Spanish\n"
                        + "\n"
                        + "2. Business cards to vCard, xml and csv\n"
                        + "  java com.net.TestApp busCard image.jpg result.xml\n"
                        + "\n"
                        + "3. Printed and handprinted text snippets\n"
                        + "  java com.net.TestApp textField image.jpg result.xml\n"
                        + "\n"
                        + "4. Barcodes\n"
                        + "  java com.net.TestApp barcode image.jpg result.xml\n"
                        + "\n"
                        + "5. Many different snippets on document\n"
                        + "  java com.net.TestApp processFields image1.jpg image2.jpg image3.tif settings.xml result.xml\n"
                        + "\n"
                        + "6. Machine-Readable Zones (MRZ) of Passports, ID cards, Visas and other official documents\n"
                        + "  java com.net.TestApp MRZ image.jpg result.xml\n"
                        + "\n"
                        + "7. Receipts\n"
                        + "  java com.net.TestApp receipt image.jpg result.xml\n"
                        + "\n"
                        + "For detailed help, call\n"
                        + "  java com.net.TestApp help <mode>\n"
    ...
    private static void displayRecognizeHelp() {
        System.out
                .println("Recognize single or multipage documents.\n"
                        + "\n"
                        + "Usage:\n"
                        + "java com.net.TestApp recognize [--lang=<languages>] <file> [<file2> ..] <output file>\n"
                        + "\n"
                        + "Output format is selected by output file extension. Possible values are:\n"
                        + ".txt, .xml, .pdf, .docx, .rtf\n"
                        + "\n"
                        + "Examples:\n"
                        + "java com.net.TestApp recognize image.tif result.txt\n"
                        + "java com.net.TestApp recognize --lang=French,Spanish page1.png page2.png page3.png result.pdf\n"
                        + "java com.net.TestApp recognize --lang=Japanese image.jpg output.rtf\n");
    }
    ...
    private static void performRecognition(Vector<String> argList)
        ...
        if (argList.size() == 1) {
            ...
            task = restAbbyyClient.processImage(argList.elementAt(0), settings);
            ...
            for (int i = 0; i < argList.size(); i++) {
                ...
                Task result = restAbbyyClient.submitImage(argList.elementAt(i),
                        taskId);
                if (task == null) {
                    task = result;
                }
            }
            task = restAbbyyClient.processDocument(task.Id, settings);
    ...
    private static void displayBusCardHelp() {
        System.out
                .println("Recognize single business card.\n"
                        + "\n"
                        + "Usage:\n"
                        + "java com.net.TestApp busCard [--lang=<languages>] <file> <output file>\n"
                        + "\n"
                        + "Output format is selected by output file extension. Possible values are:\n"
                        + ".vcf, .xml, .csv\n"
                        + "\n"
                        + "Examples:\n"
                        + "java com.net.TestApp busCard image.tif result.vcf\n"
                        + "java com.net.TestApp busCard --lang=French,Spanish image.png result.xml\n");
    }
    ...
    private static void performBusinessCardRecognition(Vector<String> argList)
        ...
        Task task = restAbbyyClient.processBusinessCard(argList.elementAt(0),
                settings);
        waitAndDownloadResult(task, outputPath);
    }
    ...
    private static void performReceiptProcessing(Vector<String> argList)
        ...
        System.out.println("Uploading..");
        Task task = restAbbyyClient.processReceipt(argList.elementAt(0), settings);
        waitAndDownloadResult(task, outputPath);
    }
    ...
    private static void displayTextFieldHelp() {
        System.out
                .println("Recognize printed or handprinted text field.\n"
                        + "\n"
                        + "Usage:\n"
                        + "java com.net.TestApp textField [--lang=<languages>] [--options=<options] <file> <output file>\n"
                        + "\n"
                        + "<options> - options passed directly to processTextField RESTful call\n"
                        + "\n"
                        + "Examples:\n"
                        + "java com.net.TestApp textField image.tif result.xml\n"
                        + "java com.net.TestApp textField --options='letterSet=0123456789/&regExp=[0-9][0-9]' image.tif result.xml\n");
    }
    ...
    private static void performTextFieldRecognition(Vector<String> argList)
        ...
        System.out.println("Uploading..");
        Task task = restAbbyyClient.processTextField(argList.elementAt(0), settings);
    ...
    private static void displayBarcodeHelp() {
        System.out
                .println("Recognize barcode.\n" + "\n" + "Usage:\n"
                        + "java com.net.TestApp barcode <file> <output file>\n" + "\n"
                        + "Examples:\n"
                        + "java com.net.TestApp barcode image.tif result.xml\n");
    }
    ...
    private static void performBarcodeRecognition(Vector<String> argList)
        ...
        System.out.println("Uploading..");
        Task task = restAbbyyClient.processBarcodeField(argList.elementAt(0),
                settings);
    ...
    private static void displayProcessFieldsHelp() {
        System.out
                .println("Process different snippets in one- or multipage document.\n"
                        + "\n"
                        + "Usage:\n"
                        + "java com.net.TestApp processFields <file1> [file2 ..] <settings.xml> <output file>\n"
                        + "\n"
                        + "For details how to create xml settings see\n"
                        + "https://ocrsdk.com/documentation/specifications/xml-scheme-field-settings/\n"
                        + "\n"
                        + "Examples:\n"
                        + "java com.net.TestApp processFields image1.tif settings.xml result.xml\n"
                        + "java com.net.TestApp processFields image1.tif image2.tif image3.tif settings.xml result.xml\n");
    }
    ...
    private static void performFieldsRecognition(Vector<String> argList)
        ...
        for (int i = 0; i < argList.size(); i++) {
            ...
            Task result = restAbbyyClient.submitImage(argList.elementAt(i), taskId);
            if (task == null) {
                task = result;
            }
        }
        ...
        task = restAbbyyClient.processFields(task.Id, settingsPath);
    ...
    private static void displayProcessMrzHelp() {
        System.out
                .println("Recognize Machine-Readable Zones of official documents\n"
                        ...
                        + "\n" + "Usage:\n"
                        + "java com.net.TestApp MRZ <file> <output file.xml>\n" + "\n");
    }
    ...
    private static void displayProcessReceiptHelp() {
        System.out
                .println("Process receipt\n"
                        + "\n" + "Usage:\n"
                        + "java com.net.TestApp receipt <file> <output file.xml>\n" + "\n");
    }
    ...
    private static void performMrzRecognition(Vector<String> argList)
        ...
        System.out.println("Uploading..");
        Task task = restAbbyyClient.processMrz(argList.elementAt(0));
    ...
    private static Task waitForCompletion(Task task) throws Exception {
        ...
        while (task.isTaskActive()) {
            ...
            task = restAbbyyClient.getTaskStatus(task.Id);
        }
        return task;
    }
    ...
    private static AbbyyClient restAbbyyClient;
}
